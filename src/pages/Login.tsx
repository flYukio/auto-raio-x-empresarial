...
export function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Se logar com sucesso, redireciona para o dashboard
      navigate('/dashboard');
    } catch (err: any) {
      ...
      >
        <div className="glass-panel p-8 sm:p-10 rounded-3xl border border-border-subtle shadow-2xl relative overflow-hidden">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-text-secondary text-sm">Acesse o motor de decisão da sua empresa.</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            <div>
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  required
                  className="w-full bg-element-bg border border-border-subtle rounded-xl py-3.5 pl-11 pr-4 text-text-primary focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors" 
            ...
            <div>
              <div className="flex justify-between items-center mb-1.5">
                ...
                <a href="#" className="text-xs text-primary hover:text-blue-400 transition-colors">Esqueceu a senha?</a>
              </div>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
            ...
            <button 
              type="submit" 
              disabled={loading}
              className="w-full py-3.5 rounded-xl bg-text-primary text-background font-bold hover:opacity-90 transition-all flex items-center justify-center gap-2 mt-8 disabled:opacity-70 disabled:cursor-not-allowed"
            >
              {loading ? (
          ...
          <div className="mt-8 text-center text-sm text-text-secondary">
            Não tem uma conta? <Link to="/signup" className="text-primary hover:text-blue-400 font-medium transition-colors">Solicite acesso</Link>
          </div>
        </div>
      </motion.div>
    </div>
